...
public class MessageDaoImpl implements IMessageDao {
	...
	public List<Message> getMessageByConversation(Conversation conversation) throws Exception {
		return (ArrayList<Message>) this.run(PropertyHelper.GetMongoDBChat(), "Message", collection -> {
			Document con1 = this.createFilterConversation(conversation.getSenderId(), conversation.getReceiverId());
			Document con2 = this.createFilterConversation(conversation.getReceiverId(), conversation.getSenderId());
			Document filter = Document.parse(String.format("{$or: [%s, %s]}", con1.toJson(), con2.toJson()));
