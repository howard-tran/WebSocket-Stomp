...
export class Message extends Component<{}, MessageState> {
    ...
    promise.then((res: any) => {
      ...
      if (_res.status != 200) {
        ...
        arr.data.map((x) => {
          ...
          if (x.sender == user.userName) {
            this.messageAreaIns.insertBefore(
              this.generateMyMessage(x.content),
              this.messageAreaIns.firstChild
            );
          } else {
            this.messageAreaIns.insertBefore(
              this.generateOtherMessage(x.content),
              this.messageAreaIns.firstChild
            );
          }
        });
  ...
  displayReceiver() {
    ...
    (this.messageAreaIns.previousSibling.firstChild as HTMLElement).innerHTML =
      "🗣️ " + this.state.messageReciever;
  }
  ...
  receiveMessageHandler = (message: IMessage) => {
    ...
    if ((messageBody.data as IMessage_t).sender == this.state.messageReciever) {
      this.messageAreaIns.appendChild(
        this.generateOtherMessage((messageBody.data as IMessage_t).content)
      );
  ...
  render() {
    return (
      <div className="message-main">
        ...
        <div
          className="message-mess-area"
          ref={(ins) => {
            this.messageAreaIns = ins;
          }}
        ...
        <div className="message-chat-area">
          <textarea
            rows={2}
            ref={(ins) => {
              this.messageTxtIns = ins;
            }}
            onKeyDown={this.textAreaHandle}
          ></textarea>
          <button type="button" className="btn btn-lg btn-primary" onClick={this.submitMessage}>
            Submit
          </button>
        </div>
      </div>
